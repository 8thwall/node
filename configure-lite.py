...
tools_path = Path('external/node/tools')
...
# imports in tools/
sys.path.insert(0, 'external/node/tools')
...
parser.add_argument('--config-gypi-output',
    action='store',
    dest='config_gypi_output',
    default='',
    help='Location of the output config.gypi file.')
...
def configure_node_lib_files(o):
  strip_prefix = 'external/node/'
  o['variables']['node_library_files'] = [
    x.removeprefix(strip_prefix) for x in SearchFiles('{}lib'.format(strip_prefix), 'js')]
...
# configure_library('zlib', output)  # FIXME Fix zlib-chromium dependency or use node's.
...
write(options.config_gypi_output, do_not_edit +
